...
import { findUserByEmail, getRegisteredContacts, searchContacts, syncContacts } from './contacts'
...
const messageResponseSchema = {
  ...
  required: ['id', 'conversationId', 'senderId', 'content', 'messageType', 'isRead', 'isDelivered', 'createdAt', 'sender'],
};
...
const getMessagesSchemas = {
  ...
  response: {
    ...
    404: { // "Current user not found" ou "Conversation not found"
      type: 'object',
      properties: {
        message: { type: 'string' },
      },
...
const sendMessageSchemas = {
  ...
  response: {
    201: { // Resposta de Sucesso (Mensagem Criada)
      type: 'object',
      properties: {
...
const markMessageAsReadSchemas = {
  ...
  response: {
    ...
    404: { // "Current user not found" ou "Message not found"
      type: 'object',
      properties: {
        message: { type: 'string' },
      },
...
const searchContactsSchemas = {
  ...
  response: {
    ...
    400: { // Erro de Validação Zod (q não fornecido ou vazio)
      type: 'object',
      properties: {
        message: { type: 'string' },
        errors: { type: 'array', items: { type: 'object' } },
...
const findUserByEmailSchemas = {
  ...
  response: {
    200: { // Resposta de Sucesso (Usuário Encontrado)
      type: 'object',
      properties: {
    ...
    400: { // Erro de Validação Zod ou "Cannot add yourself as a contact"
      type: 'object',
      properties: {
        message: { type: 'string' },
        errors: { type: 'array', items: { type: 'object' } }, // Para erros Zod
    ...
    404: { // "Current user not found" ou "User not found"
      type: 'object',
      properties: {
        message: { type: 'string' },
